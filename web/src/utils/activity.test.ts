...
import { buildStaticEvents, buildLiveEvents, DEFAULT_EVENT_LIMIT } from './activity';
  ...
  describe('buildLiveEvents', () => {
    ...
    it('maps PullRequestEvent correctly (merged)', () => {
      const raw = [{
        ...
        payload: {
          action: 'closed',
          pull_request: { number: 6, title: 'New PR', html_url: 'pr-url', merged: true }
        }
      }];
      const events = buildLiveEvents(raw, fallbackUrl);
      expect(events[0]).toMatchObject({
    ...
    it('respects maxEvents limit', () => {
      const raw = [
        { id: '1', type: 'IssuesEvent', created_at: '2026-02-05T12:00:00Z', payload: { issue: { number: 1 }, action: 'opened' } },
        { id: '2', type: 'IssuesEvent', created_at: '2026-02-05T11:00:00Z', payload: { issue: { number: 2 }, action: 'opened' } },
      ];
      // @ts-ignore
