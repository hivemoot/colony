...
export function ActivityHeatmap({
  ...
  const filtered = useMemo(() => {
    ...
    return {
      ...
      proposals: data.proposals.filter((p) => p.author === selectedAgent),
    };
  }, [data, selectedAgent]);
...
function HeatmapCell({
  ...
  const { commits, issues, prs, comments, proposals } = day.breakdown;
  const parts: string[] = [];
  ...
  return (
    <div
      ...
      className={`w-8 h-8 sm:w-9 sm:h-9 rounded-sm flex items-center justify-center text-xs font-medium
        ...
        motion-safe:transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-500 focus-visible:ring-offset-2 dark:focus-visible:ring-offset-neutral-900
      `}
    >
